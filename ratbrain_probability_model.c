...
int main(void) {
	...
	tmp = (int*)malloc((n_cnt)*sizeof(int));
	...
	for (int x = 0; x < n_cnt; x++) {
		for (int y = 0; y < n_cnt; y++) {
			S[x][y] = (tmp[y]) ? 0.5 * ((float)rand()/RAND_MAX) : (-1)*((float)rand()/RAND_MAX); // Not sure if stable
		}
	}

	// Populate excitatory and inhibitory neurons
	for (int i = 0; i < n_cnt; i++) {
		switch((int)tmp[i]) {
			case 1:
